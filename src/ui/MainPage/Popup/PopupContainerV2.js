...
class PopupContainer extends Component {
  ...
  componentDidMount() {
    this._generatePopupContainer();
  }

  componentDidUpdate(prevProps, prevState) {
    if (prevProps.popupReducer.type !== this.props.popupReducer.type) {
      this._generatePopupContainer();
    }
  }


  _generatePopupContainer = () => {
    const { isShowLv1, isShowLv2 } = this.props.popupReducer
  ...
  componentWillReceiveProps(n) {
    if (n) {
      if (n.popupReducer) {
        console.log(n.popupReducer.type);
          ...
          case types.SEND_SPLIT_CONFIRMATION:
            {
              ...
              let contentSuccess = (
                <FormSplitConfirmation
                  transaction={n.popupReducer.transaction}
                  back={() => {
                    this.setState({ contentSuccess: null }, () => {
                      if (imgTrans && blockColor && blockForm) {
                        blockColor.style.width = "261px";
                        imgTrans.style.opacity = 1;
                        blockForm.style.opacity = 1;
              ...
              if (imgTrans && blockColor && blockForm) {
                blockColor.style.width = "0";
                imgTrans.style.opacity = 0;
                blockForm.style.opacity = 0;
              }
              setTimeout(() => {
                console.log(contentSuccess);
  ...
  render() {
    let { data } = this.props;
    let { isShow, content, contentSuccess } = this.state;
    console.log(this.state);

    return (
      <React.Fragment>
        <Bound styled={data.style}>
          <div className={isShow ? "popup-container show" : "popup-container"}>
